// src/pages/rds/RDSResearchMonitoring.jsx
...
const dummySubmissions = Array.from({ length: 50 }, (_, i) => ({
  ...
  task:
    i % 3 === 0
      ...
      : 'Ethics Review Attachment',
  submitter: i % 2 === 0 ? 'Dr. Ada Lovelace' : 'Dr. Alan Turing',
  ...
  file: i % 3 === 1 ? '' : `rds_submission_${i + 1}.pdf`,
...
const RDSResearchMonitoring = ({ submissions = dummySubmissions }) => {
  ...
  const filtered = submissions.filter((entry) => {
    const matchText = (entry.task + entry.submitter)
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchStatus = statusFilter ? entry.progress === statusFilter : true;

    const date = entry.submittedDate ? new Date(entry.submittedDate) : null;
    const inRange =
      (!startDate || (date && date >= new Date(startDate))) &&
      (!endDate || (date && date <= new Date(endDate))) &&
      !!entry.submittedDate;

    return matchText && matchStatus && inRange;
  });
  ...
  return (
    <div className="admin-dashboard">
      <h1 className="page-title">RDS Research Monitoring</h1>
...
export default RDSResearchMonitoring;
