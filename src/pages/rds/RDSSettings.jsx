...
const RDSSettings = () => {
  ...
  // Same field layout as ETS (separate name fields + address)
  ...
  // Load profile (includes address keys if columns exist)
  useEffect(() => {
    let mounted = true;
    (async () => {
      if (!userId) {
      ...
      setLoadErr('');
      try {
        const res = await fetch(`${API_BASE}/api/rds/settings/${userId}`, { headers: headersAuth() });
  ...
  const saveProfile = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/rds/settings/${userId}`, {
        method: 'PATCH',
        headers: headersAuth(),
  ...
  const changePassword = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/rds/settings/${userId}/password`, {
        method: 'PATCH',
        headers: headersAuth(),
        body: JSON.stringify({
  ...
  return (
    <div className="admin-dashboard">
      <h1 className="page-title">RDS Account Settings</h1>
