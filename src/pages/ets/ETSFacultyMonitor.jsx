// src/pages/ets/ETSFacultyMonitor.jsx
...
const ETSFacultyMonitor = () => {
  ...
  // Load ETS faculty list (non-admin)
  useEffect(() => {
    let mounted = true;
    (async () => {
      setLoading(true);
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/api/ets/faculty?${params}`, { headers });
              ...
              {rows.length === 0 ? (
                ...
                <tr key={u.id}>
                  ...
                  <td style={{ display: 'flex', gap: 6 }}>
                    <button className="secondary" onClick={() => navigate(`/ets/faculty/${u.id}`)}>
                      View Profile
                    </button>
                  </td>
                </tr>
...
export default ETSFacultyMonitor;
