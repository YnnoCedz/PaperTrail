// server/routes/rds/rdsSettings.js
...
import {
  ...
  fetchUserById,
  getEditableEntries,
  getEditableFieldsForRDS,
} from '../../utils/addressApi.js';

const router = express.Router();

/** GET profile (includes address keys; null if address columns are missing). */
router.get('/settings/:id', async (req, res) => {
  try {
    ...
    console.error('RDS settings GET error:', e);
    res.status(500).json({ message: 'Server error' });
  }
});

/** Update profile (edits only columns that exist for RDS). */
router.patch('/settings/:id', async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ message: 'Invalid id' });

    const EDITABLE_FIELDS = await getEditableFieldsForRDS();
    ...
    console.error('RDS settings PATCH error:', e);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.patch('/settings/:id/password', async (req, res) => {
  try {
    ...
    console.error('RDS settings PW PATCH error:', e);
    res.status(500).json({ message: 'Server error' });
  }
});
